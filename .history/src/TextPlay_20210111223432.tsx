...
export class TextPlay extends Component<{}, TextState> {
    ...
    createTextDivs(): React.ReactNode  {
        let values = [];
        for(const [string, value] of Object.entries(this.state.text)) {
            values.push(<div onMouseOver={() => this.setState({ textState: value })} onMouseOut={() => this.setState({ textState: this.state.defaultState })}>
            {
